...
### Added (v0.2.0)
- Release-workflow supply-chain artifacts: CycloneDX SBOM (`fpagent-<version>.cdx.json`) and Sigstore keyless signatures (`*.sigstore.json`) are produced during the release job and attached to the GitHub Release alongside the sdist and wheel.
- `docs/supply-chain.md` walks through verifying a wheel against its Sigstore bundle using `sigstore verify github`.

### Omitted (v0.2.0)
- SLSA build provenance. Evaluated and deferred: the Sigstore keyless signature already encodes the workflow identity, so SLSA would duplicate the same trust fact for a pre-1.0 alpha with no enterprise consumers. Revisit at 1.0.
- PyPI publication. Per maintainer decision; install remains `pip install git+https://…`.
